...
export var CityPicker = (function() {
  ...
  CityPicker.prototype.generate = function(picker) {
    ...
    var provinceCol = {
      name: 'province',
      options: this.citiesData.map(function(province) { return { text: province.name, value: province.id, disabled: false }; })
    };
    ...
    var cityCol = {
      name: 'city',
      options: cityColData.map(function(city) { return { text: city.name, value: city.id, disabled: false }; })
    };
    ...
    var regionColCol = {
      name: 'region',
      options: regionData.map(function(city) { return { text: city.name, value: city.id, disabled: false }; })
    };
  ...
  CityPicker.prototype.validate = function(picker) {
    ...
    if (cityCol && this._provinceCol != provinceCol.selectedIndex) {
      ...
      var cityColData = this.citiesData[provinceCol.selectedIndex].children;
      cityCol.options = cityColData.map(function(city) { return { text: city.name, value: city.id, disabled: false }; });
    ...
    if (regionCol && (this._cityCol != cityCol.selectedIndex || this._provinceCol != provinceCol.selectedIndex)) {
      ...
      regionCol.options = regionData.map(function(city) { return { text: city.name, value: city.id, disabled: false }; });
